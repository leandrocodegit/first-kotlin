    ...
    fun updateStatusProduct(id: Long, statusProduct: StatusProduct): Product{
        ...
        if(product.isActive == statusProduct.value)
            when(product.isActive){
                StatusProduct.ACTIVE.value -> throw EntityExecption("Produto ja esta ativo", CodeError.NOT_FOUND)
                StatusProduct.INACTIVE.value -> throw EntityExecption("Produto ja esta inativo", CodeError.NOT_FOUND)
            }
